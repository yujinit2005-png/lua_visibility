...
- **웹 실측 전체 질문 순차 자동 실측 1:1 완전 동기화 (`WebVerificationModal.tsx`)**:
  - 이전에는 7초 타이머로 병렬 트리거되어 브라우저가 다 닫히지 않은 상태에서 다음 창이 탭으로 끼어들거나 프로필 락(500 에러)이 발생하던 문제를 해결.
  - 이전 질문의 크롤링 완료 및 브라우저 창 종료(`context.close()`)를 `await`로 100% 보장한 후 다음 질문 창을 순차 실행하도록 변경.
